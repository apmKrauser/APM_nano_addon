...
#define FrontLight_brightness 120  // 0-255
...
boolean      SW2 = false;
uint16_t     rpm = 0;
uint16_t     rpms[RPM_avg_Nr] = {0};
Servo		 ModeOut;
uint8_t      cntRPM_times = 0;
uint16_t     RPM_times[15] = {0};
...
void updateRPM()
{
	...
	if (timeRPM != 0) 
	{
		...
		RPM_times[cntRPM_times] = (uint16_t) (timeRPM / 10);
		cntRPM_times++;
		if ( cntRPM_times > 14 ) cntRPM_times = 9;
	}
	int_detached = true;
...
void RPM_detach()
{
			...
			timeRPM = 0;
			if (cntRPM_times > 0) {
				for (uint8_t i = 0; i < cntRPM_times; i++)
				{
					timeRPM += ((uint32_t) RPM_times[i]);

				}
				//timeRPM *= 10;
				timeRPM = (unsigned long) (timeRPM / ((unsigned long) cntRPM_times));
				if (timeRPM != 0) {
					ri = RPM_avg_Nr - 1;
					rpm = 0;
					while ( ri ) { 
					...
					rpms[0] = (uint16_t) ((6000000UL / (timeRPM)) / 2);  // /2 don't know why atm
					rpm += rpms[0];
					rpm = rpm / RPM_avg_Nr;
				}
			}
